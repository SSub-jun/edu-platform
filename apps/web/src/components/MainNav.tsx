    ...
    <header className="w-full border-b border-border bg-surface/80 backdrop-blur-sm">
      <div className="max-w-6xl mx-auto flex items-center justify-between px-4 py-3 md:py-4">
        <Link href="/curriculum" className="flex items-center gap-2">
          <span className="text-base md:text-lg font-semibold text-text-primary tracking-tight">
            한국산업보건안전기술원 교육센터
          </span>
        </Link>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`px-3 py-1.5 md:px-4 md:py-2 rounded-full font-medium transition-colors ${
                  isActive
