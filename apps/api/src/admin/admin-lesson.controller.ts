...
export class AdminLessonController {
  ...
  @ApiResponse({ status: 201, description: '과목이 성공적으로 생성됨' })
  async createSubject(@Body() dto: CreateSubjectDto) {
    const subject = await this.lessonService.createSubject(dto);
  ...
  async getSubjects() {
    const subjects = await this.lessonService.getSubjects();
  ...
  @ApiResponse({ status: 200, description: '과목 상세 정보' })
  async getSubjectDetail(@Param('subjectId') subjectId: string) {
    const subject = await this.lessonService.getSubjectDetail(subjectId);
    return { success: true, data: subject };
  }
  ...
  async updateSubject(
    ...
    const subject = await this.lessonService.updateSubject(subjectId, dto);
    return { success: true, data: subject };
  }
  ...
  async deleteSubject(@Param('subjectId') subjectId: string) {
    return this.lessonService.deleteSubject(subjectId);
  }
  ...
  async createLessonPart(
    @Param('lessonId') lessonId: string,
    @Body() dto: CreateLessonPartDto,
  ) {
  ...
  @ApiResponse({
    status: 200,
    description: '레슨 파트가 성공적으로 비활성화됨',
  })
  ...
  @ApiResponse({
    status: 200,
    description: '시험 문제가 성공적으로 비활성화됨',
  })
